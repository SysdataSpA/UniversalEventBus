...
public final class RxEventProcessor implements EventProcessor {
    ...
    private RxEventProcessor() {
        // No instances.
        BUS.observeOn(Schedulers.newThread()).subscribeOn(Schedulers.newThread()).toFlowable(BackpressureStrategy.BUFFER).onBackpressureBuffer();
        UI_BUS.observeOn(Schedulers.newThread()).subscribeOn(AndroidSchedulers.mainThread()).toFlowable(BackpressureStrategy.BUFFER).onBackpressureBuffer();
    }
    ...
    public void setVerbose(boolean verbose) {
        RxEventProcessor.verbose = verbose;
    }
     ...
     */
    static final class ObserverWrapper implements Observer {
